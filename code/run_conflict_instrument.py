...
THD = 0.55
N_RUNS = int(os.environ.get("N_RUNS", 15))   # override with N_RUNS=5 python ... for scale-expansion runs
...
def load_config(network, axis):
    ...
    comparable to prior JSONs:
        ...
        kanno markov    -> run_markov_2x2.py    used 800_000
        alarm tree      -> 1_000_000 (new)
        alarm markov    -> 1_010_000 (new)
    """
    if network == "domain17":
        from domain17 import make_domain17, make_observability17
        spec = make_domain17()
        obs = make_observability17()
        pool_path = "/home/claude/substrate_abm/hard_scenarios_domain17.pkl"
        ...
        pool_path = "/home/claude/substrate_abm/hard_scenarios.pkl"
        base_seed = {"tree": 500_000, "markov": 800_000}[axis]
    elif network == "alarm":
        from alarm_domain import make_alarm, make_observability_alarm
        spec = make_alarm()
        obs = make_observability_alarm()
        pool_path = "./results/hard_scenarios_alarm.pkl"
        base_seed = {"tree": 1_000_000, "markov": 1_010_000}[axis]
    else:
...
def main():
    if (len(sys.argv) != 3
        ...
        print("Usage: python run_conflict_instrument.py {domain17|kanno|alarm} {tree|markov}")
        sys.exit(1)
    network, axis = sys.argv[1], sys.argv[2]

    spec, obs, scenarios, base_seed = load_config(network, axis)

    # Output path: use ./results/ for alarm, existing convention for others
    if network == "alarm":
        out_path = f"./results/conflict_instrument_{network}_{axis}.json"
    else:
        out_path = f"/home/claude/substrate_abm/conflict_instrument_{network}_{axis}.json"
