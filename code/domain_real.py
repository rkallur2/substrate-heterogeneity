...
def make_observability_real_n5() -> dict:
    """Team-size scalability partition for Kanno: 5 agents, 2 obs/agent,
    2 overlaps. 8 unique observed nodes (GasTank and Lights moved from
    observable to hidden inference targets, so total hidden goes from 7
    to 9). Matches the structural pattern of Domain17 N=5.

    Roles:
      A (Electrical primary):        Alter, BatVolt
      B (Ignition):                  Plugs, MFuse
      C (Fuel supply):               Fuel, MFuse  -- overlap with B
      D (Intake filters):            AirFilter, GasFilter
      E (Starter circuit):           StMotor, BatVolt  -- overlap with A
    """
    return {
        "A": ["Alter", "BatVolt"],
        "B": ["Plugs", "MFuse"],
        "C": ["Fuel", "MFuse"],       # overlap with B on MFuse
        "D": ["AirFilter", "GasFilter"],
        "E": ["StMotor", "BatVolt"],  # overlap with A on BatVolt
    }
